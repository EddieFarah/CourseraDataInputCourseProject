...
# Combine These columns : subjects, labels, everything else
...
# keep features from mean and standard deviation only
features.mean.std <- features[grep("mean\\(\\)|std\\(\\)", features$V2), ]

# select the means and standard deviations from data only
...
# replace with label names
data.mean.std$label <- labels[data.mean.std$label, 2]

## step 4
# first make a list of column names and feature names
...
write.table(format(aggr.data, scientific=T), "UCI HAR Dataset/tidy2.txt",
